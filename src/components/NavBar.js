...
const NavBar = () => {
  ...
  return (
    <div className='flex justify-between items-center w-full h-20 text-white px-4 bg-black fixed'>
        ...
        {
            links.map((link,index)=><li key={index} className="px-4 cursor-pointer capitalize font-medium text-gray-500 hover:scale-105 duration-200"><Link to={link}>{link}</Link></li>)
        }
        </ul>
        ...
        {
            links.map((link,index)=><li key={index} className="px-4 cursor-pointer capitalize py-6 text-4xl"><Link to={link}>{link}</Link></li>)
        }
        </ul>
            )
        }
